...
def pagina_inicio():
    ...
    st.write("Bienvenido a la aplicación multi-página con Streamlit.")

# Función para la página de gráficas
def pagina_graficas():
    st.title("Gráficas Interactivas y Estáticas")
    
    # --- Gráfica con Matplotlib ---
    st.subheader("Gráfica Estática con Matplotlib")
    ...
    st.subheader("Gráfica Interactiva con Plotly")
    df = pd.DataFrame({"x": x, "y": y})
    fig_plotly = px.line(df, x="x", y="y", title="Ejemplo Plotly")
    st.plotly_chart(fig_plotly)

# Función para la página de generación de documento Word
def pagina_documento():
    st.title("Generación de Documento Word")
    st.write("Se generará un documento Word con una gráfica insertada.")
    
    # Generar gráfica con Matplotlib y guardarla en un buffer
    fig_mpl, ax = plt.subplots()
    x = [1, 2, 3, 4]
    y = [10, 20, 25, 30]
    ax.plot(x, y, marker='o')
    ax.set_title("Gráfica para el Documento")
    
    buffer = io.BytesIO()
    fig_mpl.savefig(buffer, format='png')
    buffer.seek(0)  # Reinicia el buffer para poder leerlo posteriormente

    # Cargar el template de Word (asegúrate de tener un template.docx en tu directorio)
    try:
        doc = DocxTemplate("templates/resultado.docx")
    except Exception as e:
        st.error("No se encontró el template.docx o hubo un error al cargarlo.")
        st.error(e)
        return

    # Crear un objeto InlineImage a partir del buffer
    image_mpl = InlineImage(doc, buffer, width=Mm(100))

    # Crear el diccionario de contexto para reemplazar en el template
    context = {
        "grafica_mpl": image_mpl,
        # Aquí puedes agregar más llaves y contenido para el template
    }
    ...
    st.download_button("Descargar Documento Word", data=doc_buffer, file_name="resultado_Automatizado.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        ...
        try:
            ...
            if st.button("Generar Informe Automatizado", type="primary"):
                ...
                try:
                    ...
                    columnas_Energia_Activa_Unidades = obtener_Columnas_DataFrame(dataFrame=df, nombres_Fijos_Columnas=['Eptot+(Med)', 'Eptot-(Med)'], valores_Corchetes=['kWh'])
                    ...
                    columnas_Energia_Inductiva_Unidades = obtener_Columnas_DataFrame(dataFrame=df, nombres_Fijos_Columnas=['EQtotind+(Med)', 'EQtotind-(Med)'], valores_Corchetes=['kvarh'])
                    ...
                    columnas_Energia_Capacitiva_Unidades = obtener_Columnas_DataFrame(dataFrame=df, nombres_Fijos_Columnas=['EQtotcap+(Med)', 'EQtotcap-(Med)'], valores_Corchetes=['kvarh'])
...
def main():
    st.sidebar.title("Menú de Páginas - Aplicación Web (Metrel V1)")
